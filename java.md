...
- [String, StringBuilder, StringBuffer](#string-stringbuilder-stringbuffer)
  - [`String` (неизменяемый)](#string-неизменяемый)
  - [`StringBuilder` (изменяемый, не потокобезопасный)](#stringbuilder-изменяемый-не-потокобезопасный)
  - [`StringBuffer` (изменяемый, потокобезопасный)](#stringbuffer-изменяемый-потокобезопасный)
- [Работа с памятью](#работа-с-памятью)
  - [Пул строк (String Pool)](#пул-строк-string-pool)
    - [Как строки попадают в пул?](#как-строки-попадают-в-пул)
    - [Ручное интернирование: метод `intern()`](#ручное-интернирование-метод-intern)
    - [Где находится пул строк?](#где-находится-пул-строк)
    - [Зачем нужен пул?](#зачем-нужен-пул)
    - [Примеры и подводные камни](#примеры-и-подводные-камни)
...
В Java существуют три основных класса для работы со строками: `String`, `StringBuilder` и `StringBuffer`. Они различаются по изменяемости, потокобезопасности и производительности.

### `String` (неизменяемый)

- **Неизменяемость**: После создания объект String не может быть изменён. Любая операция модификации (конкатенация, замена и т.д.) создаёт новый объект String.
- **Потокобезопасность**: Изначально потокобезопасен, так как его состояние не меняется после создания.
- **Производительность**: Неэффективен при частых изменениях, поскольку каждая операция порождает новый объект, увеличивая нагрузку на память и сборщик мусора.
- **Применение**: Идеален для неизменяемых строк, строковых констант или данных, которые не меняются (например, параметры конфигурации, ключи в коллекциях).

```java
String s = "Привет";
s = s + " мир"; // Создаётся новый объект String
```

### `StringBuilder` (изменяемый, не потокобезопасный)

- **Изменяемость**: Позволяет изменять содержимое объекта (добавлять, вставлять, удалять) без создания новых объектов.
- **Потокобезопасность**: Не синхронизирован – не предназначен для одновременного доступа из нескольких потоков.
- **Производительность**: Самый быстрый среди трёх в однопоточных сценариях, так как отсутствуют накладные расходы на синхронизацию.
- **Применение**: Лучший выбор для манипуляций со строками внутри одного метода или потока (например, построение большой строки в цикле).

```java
StringBuilder sb = new StringBuilder("Привет");
sb.append(" мир"); // Изменяется существующий объект
```

### `StringBuffer` (изменяемый, потокобезопасный)

- **Изменяемость**: Те же возможности модификации, что и у `StringBuilder`.
- **Потокобезопасность**: Его методы синхронизированы, поэтому он безопасен для использования в многопоточной среде.
- **Производительность**: Медленнее `StringBuilder` из-за затрат на захват и освобождение блокировок.
- **Применение**: Подходит, когда один и тот же буфер строки используется несколькими потоками (например, в разделяемых структурах данных).

```java
StringBuffer sbf = new StringBuffer("Привет");
sbf.append(" мир"); // Потокобезопасная операция
```
...
**Пул строк (String Pool)** — это специальная область памяти в куче (heap), в которой хранятся уникальные экземпляры класса `String`. Его основная цель — экономия памяти и ускорение сравнения строк за счёт повторного использования уже существующих объектов.

#### Как строки попадают в пул?

1. **Строковые литералы**  
    Когда вы объявляете строку с помощью двойных кавычек, например:
    ...
    компилятор помещает литерал `"Java"` в пул строк. При создании `s2` JVM сначала проверяет, есть ли уже такая строка в пуле. Если есть (а в нашем случае она уже там), то переменная `s2` будет ссылаться на тот же объект, что и `s1`. Таким образом, `s1 == s2` вернёт `true`, потому что это одна и та же ссылка.

2. Создание строк через `new`  
   Если вы создаёте строку оператором `new`, например:

   ```java
   String s3 = new String("Java");
   ```

   то создаётся **новый объект** в куче (вне пула), даже если такая строка уже есть в пуле. При этом литерал "Java" всё равно будет помещён в пул, но `s3` будет ссылаться на отдельный объект. Сравнение `s1 == s3` даст `false`, потому что ссылки разные.

#### Ручное интернирование: метод `intern()`

Чтобы принудительно поместить строку в пул (или получить ссылку на строку из пула), используется метод `intern()`:

```java
String s4 = new String("Java");
String s5 = s4.intern(); // теперь s5 ссылается на строку из пула
```

Если в пуле уже есть строка с таким содержимым, `intern()` вернёт ссылку на неё. Если нет — добавит текущую строку в пул и вернёт ссылку на неё.

#### Где находится пул строк?

- До Java 7 пул строк располагался в области памяти PermGen (Permanent Generation), которая имела фиксированный размер и могла вызывать `OutOfMemoryError` при переполнении.
- Начиная с Java 7 пул строк перенесён в обычную **кучу (heap)**. Это позволило пулу динамически расти и использовать сборку мусора для удаления неиспользуемых строк.

#### Зачем нужен пул?

- Экономия памяти: повторное использование одинаковых строк вместо создания множества дубликатов.
- Быстрое сравнение: если вы уверены, что обе строки интернированы, их можно сравнивать через `==` (сравнение ссылок), что быстрее, чем `equals()`. Однако это рискованный подход, если строки получены из разных источников.
...
System.out.println(a == c); // false (c не из пула)

String d = c.intern();
System.out.println(a == d); // true (d теперь ссылается на пул)

String e = "he" + "llo"; // константа, вычисляется на этапе компиляции
System.out.println(a == e); // true

String f = "he";
String g = f + "llo"; // вычисляется во время выполнения, создаётся новый объект
...
`@UtilityClass` — аннотация для создания класса-утилиты. Автоматически делает класс `final`, добавляет приватный конструктор и делает все методы и поля `static`.

| Элемент                  | Что делает                                                              |
|--------------------------|-------------------------------------------------------------------------|
| `final` на классе        | Запрещает наследование                                                  |
| Приватный конструктор    | Бросает `UnsupportedOperationException` при попытке создать экземпляр   |
| `static` на всех членах  | Все методы и поля становятся статическими автоматически                 |
