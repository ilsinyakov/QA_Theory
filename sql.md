...
- [Команды SQL](#команды-sql)
  - [Порядок выполнения SQL (логический)](#порядок-выполнения-sql-логический)
...
- [Выражения](#выражения)
  ...
  - [Агрегирующие вычисления](#агрегирующие-вычисления)
    - [Выражение `FILTER`](#выражение-filter)
...
- [Хранимые процедуры](#хранимые-процедуры)
  - [Разница между функциями и хранимыми процедурами](#разница-между-функциями-и-хранимыми-процедурами)
  - [Примеры хранимых процедур](#примеры-хранимых-процедур)
    - [Для добавления пользователя](#для-добавления-пользователя)
    - [Для обновления баланса счета](#для-обновления-баланса-счета)
- [Триггеры](#триггеры)
  - [Основные компоненты триггера](#основные-компоненты-триггера)
    - [Время срабатывания](#время-срабатывания)
    - [События](#события)
    - [Уровень](#уровень)
    - [Переменные внутри триггерной функции](#переменные-внутри-триггерной-функции)
  - [Примеры триггеров](#примеры-триггеров)
    - [Для логирования изменений](#для-логирования-изменений)
    - [Для автоматического обновления timestamp](#для-автоматического-обновления-timestamp)
    - [Для проверки данных](#для-проверки-данных)
    - [Для вычисляемого поля](#для-вычисляемого-поля)
    - [Для каскадных изменений](#для-каскадных-изменений)
...
### Порядок выполнения SQL (логический)

Стандартный порядок обработки:

1. `FROM` / `JOIN` — определяются исходные таблицы.
2. `WHERE` — фильтрация строк.
3. `GROUP BY` — группировка строк по указанным столбцам.
4. `HAVING` — фильтрация групп.
5. `SELECT` — вычисление выражений и псевдонимов.
6. `ORDER BY` — сортировка.

В некоторых диалектах алиасы из `SELECT` могут быть доступны в `GROUP BY` (например, в PostgreSQL) и в `HAVING` (например, в MySQL).
...
В записи не должно быть столбцов с неключевыми значениями, которые зависят от других неключевых значений (нет транзитивных зависимостей).
...
|Тип данных|От|До|
|--|--|--|
|bigint / int8|-9,223,372,036,854,775,808|9,223,372,036,854,775,807|
|int / int4|-2,147,483,648|2,147,483,647|
|smallint / int2|-32,768|32,767|
|tinyint|0|255|
|bit|0|1|
|decimal / numeric|-10^38 +1|10^38 -1|
...
|BLOB (Binary Large Object)|Предназначен для хранения больших объемов двоичных данных: изображений, видео, мультимедийных файлов и т.д.|
...
|rowid|Это псевдостолбец, который описывает точное физическое расположение строки.|
...
|BETWEEN|Проверяет вхождение значения в диапазон от минимального до максимального (границы включаются)|
...
Во многих диалектах, нарпример, в PostgreSQL, SQLite для фильтрации записей, попадающих в агрегатную функцию, можно использовать выражение `FILTER`.
...
`FILTER (WHERE condition)` ставится сразу после **агрегатной функции**.

Выражение `FILTER` целесообразно использовать в запросах с несколькими агрегатными функциями или, где агрегатные функции используются в вычислениях:
...
SELECT
    ROUND(
        COUNT(*) FILTER (WHERE passed) * 100.0 / COUNT(*),
        2
...
Предложение `ILIKE` делает то же самое, но не учитывает регистр букв.
...
|`WHERE col ILIKE '%foo%'`|Любые значения, содержащие `foo`, `FOO` и т.д. - независимо от регистра|
...
Синтаксис создания уникальных индексов (такие индексы используются не только для повышения производительности, но и для обеспечения согласованности данных):
...
- **атомарность** (atomicity) — неделимость: все операции транзакции должны быть успешно завершены. В противном случае, транзакция прерывается, а все изменения отменяются (происходит откат к предыдущему состоянию)
...
**Хранимая процедура** – это объект базы данных, который хранит набор инструкций языка SQL и конкретной его реализации в выбранной СУБД, реализующих определённый алгоритм.

Иными словами, функции и хранимые процедуры – это своего рода «мини программы» внутри базы данных. Они помогают реализовывать нужные нам алгоритмы и хранить их в базе данных.

### Разница между функциями и хранимыми процедурами

Хранимые процедуры похожи на пользовательские функции. Но стоит определить ключевые отличия между ними:
...
|Функции можно использовать в операторе `SELECT`|Процедуры нельзя использовать в операторе `SELECT` и во всех его секциях (`WHERE`, `JOIN`, `HAVING` и т.д.), так как процедуры вызываются с помощью команды `EXECUTE`, `EXEC` или `CALL`|
...
|В функциях можно использовать только входные параметры|В процедурах можно использовать как входные, так и выходные параметры|

В разных СУБД существует свой язык для создания хранимых процедур.  

Например:

- в Microsoft SQL Server существует язык Transact-SQL
- в Oracle — PL/SQL
- в PostgreSQL — PL/pgSQL
- MySQL достаточно близко следует стандарту SQL:2003, её язык похож на SQL/PL

Так же в некоторые СУБД поддерживают создание хранимых процедур на любом языке программирования.
...
BEGIN
    ...
    -- Вывод сообщения
    RAISE NOTICE 'Пользователь % добавлен', user_name;
END;
$$;

-- Вызов процедуры
CALL add_user('Иван Иванов', 'ivan@example.com');
CALL add_user('Мария Петрова', 'maria@example.com');
```

#### Для обновления баланса счета
...
-- Создание процедуры для пополнения счета
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Счет с ID % не найден', acc_id;
    END IF;
    
    RAISE NOTICE 'Счет % пополнен на %', acc_id, amount;
END;
$$;

-- Создание процедуры для перевода между счетами
...
BEGIN
    ...
    IF (SELECT balance FROM accounts WHERE account_id = from_acc) < amount THEN
        RAISE EXCEPTION 'Недостаточно средств на счете %', from_acc;
    END IF;
    ...
    RAISE NOTICE 'Перевод % со счета % на счет % выполнен', amount, from_acc, to_acc;
END;
$$;

-- Тестирование процедур
CALL deposit_money(1, 200.00);  -- Пополнение счета
CALL transfer_money(1, 2, 300.00);  -- Перевод между счетами
```
...
С любой операцией, вызывающей изменение содержимого таблицы, можно связать сопутствующее действие (триггер), которое СУБД должна выполнять при выполнении каждой такой операции. Триггеры могут быть связаны с выполнением операций `INSERT`, `UPDATE` и `DELETE` (или какой-то их комбинацией) по отношению к указанным таблицам.

### Основные компоненты триггера

1. **Триггерная функция** - функция, которая выполняется при срабатывании триггера
2. **Сам триггер** - привязывает функцию к событию на таблице
...
#### Для автоматического обновления timestamp
...
### Работа с временем и датами в PostgreSQL

#### Типы данных для времени

`DATE` – дата (год, месяц, день).

`TIME` – время без даты (часы, минуты, секунды с дробной частью).

`TIMESTAMP` – дата и время без учёта часового пояса.

`TIMESTAMPTZ` – дата и время с учётом часового пояса (хранится в UTC, при выводе преобразуется в локальный пояс сессии).

`INTERVAL` – временной интервал (например, 2 days 3 hours).

#### Получение текущей даты и времени

`CURRENT_DATE` – текущая дата.

`CURRENT_TIME` – текущее время с часовым поясом.

`CURRENT_TIMESTAMP` – текущие дата и время с часовым поясом.

`LOCALTIME` – текущее время без часового пояса.

`LOCALTIMESTAMP` – текущие дата и время без часового пояса.

`NOW()` – аналог CURRENT_TIMESTAMP.

`TIMEOFDAY()` – возвращает текущие дату и время как текстовую строку с миллисекундами.

```sql
SELECT CURRENT_DATE;          -- 2026-03-03
SELECT CURRENT_TIMESTAMP;     -- 2026-03-03 14:30:00.123456+03
SELECT NOW();                  -- то же самое
```

#### Извлечение частей даты/времени

Функция `EXTRACT(field FROM source)` извлекает заданную компоненту:

**field** может быть: `YEAR`, `MONTH`, `DAY`, `HOUR`, `MINUTE`, `SECOND`, `DOW` (день недели, 0=воскресенье), `DOY` (день года), `EPOCH` (количество секунд с 1970-01-01) и др.

Альтернатива – функция `DATE_PART('field', source)` с таким же поведением.

```sql
SELECT EXTRACT(YEAR FROM TIMESTAMP '2025-12-31 23:59:59');  -- 2025
SELECT EXTRACT(DOW FROM DATE '2026-03-03');                 -- 2 (вторник, если считать с воскресенья = 0)
SELECT DATE_PART('month', TIMESTAMP '2025-12-31');          -- 12
```

#### Арифметика с датами и интервалами

- Сложение/вычитание интервалов:

  ```sql
  SELECT DATE '2026-03-03' + INTERVAL '7 days';   -- 2026-03-10
  SELECT TIMESTAMP '2026-03-03 12:00' - INTERVAL '2 hours'; -- 2026-03-03 10:00
  ```

- Разность двух дат даёт целое число дней:

  ```sql
  SELECT DATE '2026-03-10' - DATE '2026-03-03';   -- 7
  ```

- Разность двух меток времени даёт интервал:

  ```sql
  SELECT TIMESTAMP '2026-03-10 12:00' - TIMESTAMP '2026-03-03 10:00'; -- 7 days 02:00:00
  ```

#### Функция `AGE()`

- Функция `AGE()` вычисляет возраст как интервал между двумя датами или между заданной датой и текущей:

  ```sql
  SELECT AGE(TIMESTAMP '2026-03-10', TIMESTAMP '2026-03-03'); -- 7 days
  SELECT AGE('2026-03-03'::DATE);                             -- возраст от этой даты до сегодня (в годах, месяцах, днях)
  ```

- Из полученного интервала можно извлечь возраст в полных годах:

  ```sql
  SELECT EXTRACT(YEAR FROM AGE(birthday::DATE))
  ```

#### Округление и усечение временных меток

`DATE_TRUNC('field', source)` усекает дату/время до заданной точности:

```sql
SELECT DATE_TRUNC('hour', TIMESTAMP '2026-03-03 14:35:22');   -- 2026-03-03 14:00:00
SELECT DATE_TRUNC('month', TIMESTAMP '2026-03-03');           -- 2026-03-01 00:00:00
SELECT DATE_TRUNC('year', DATE '2026-03-03');                 -- 2026-01-01
```

#### Форматирование и преобразование

`TO_CHAR(source, format)` – преобразует дату/время/интервал в строку по заданному формату.

```sql
SELECT TO_CHAR(CURRENT_DATE, 'DD.MM.YYYY');                 -- 03.03.2026
SELECT TO_CHAR(CURRENT_TIMESTAMP, 'HH24:MI:SS');            -- 14:30:00
```

`TO_DATE(text, format)` – преобразует строку в дату.  
`TO_TIMESTAMP(text, format)` – преобразует строку в метку времени с часовым поясом.

```sql
SELECT TO_DATE('03/03/2026', 'DD/MM/YYYY');                 -- 2026-03-03
SELECT TO_TIMESTAMP('2026-03-03 14:30:00', 'YYYY-MM-DD HH24:MI:SS');
```

#### Работа с часовыми поясами

`AT TIME ZONE` – преобразует метку времени из одного часового пояса в другой или меняет тип данных.

```sql
SELECT TIMESTAMP '2026-03-03 14:30:00' AT TIME ZONE 'UTC';  -- 2026-03-03 14:30:00+00 (становится TIMESTAMPTZ)
SELECT TIMESTAMPTZ '2026-03-03 14:30:00+03' AT TIME ZONE 'UTC'; -- 2026-03-03 11:30:00 (TIMESTAMP without TZ)
```
