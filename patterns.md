...
- [Структурные паттерны](#структурные-паттерны)
  - [Адаптер (Adapter)](#адаптер-adapter)
    ...
    - [Адаптер классов](#адаптер-классов)
  - [Декоратор (Wrapper, Обёртка, Decorator)](#декоратор-wrapper-обёртка-decorator)
...
![Class Adapter](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/class-adapter.png?raw=true)
...
![Facade](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/facade.png?raw=true)

- `Facade` предоставляет быстрый доступ к определённой функциональности подсистемы. Он «знает», каким классам нужно переадресовать запрос, и какие данные для этого нужны.
- `AdditionalFacade` можно ввести, чтобы не «захламлять» единственный фасад разнородной функциональностью. Он может использоваться как клиентом, так и другими фасадами.
...
- `Client` использует фасад вместо прямой работы с объектами сложной подсистемы.
...
![Observer1](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/observer1.png?raw=true)

Когда в издателе будет происходить событие, на которое подписаны подписчики, он будет проходиться по списку подписчиков и оповещать их об этом, вызывая определённый метод объектов-подписчиков.

Издателю безразлично, какой класс будет иметь тот или иной подписчик, так как все они должны следовать общему интерфейсу и иметь единый метод оповещения.

![Observer1](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/observer2.png?raw=true)
...
![Strategy1](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/strategy1.png?raw=true)
...
![Strategy2](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/strategy2.png?raw=true)

- `Context` хранит ссылку на объект конкретной стратегии, работая с ним через общий интерфейс стратегий.
- `Strategy` определяет интерфейс, общий для всех вариаций алгоритма. Контекст использует этот интерфейс для вызова алгоритма.
- `ConcreteStrategy` реализуют различные вариации алгоритма.
- Во время выполнения программы контекст получает вызовы от клиента и делегирует их объекту конкретной стратегии.
- `Client` должен создать объект конкретной стратегии и передать его в конструктор контекста. Кроме этого, клиент должен иметь возможность заменить стратегию на лету, используя сеттер. Благодаря этому, контекст не будет знать о том, какая именно стратегия сейчас выбрана.
