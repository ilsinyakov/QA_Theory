...
- [Git](#git)
  ...
  - [Введение в системы контроля версий. Git](#введение-в-системы-контроля-версий-git)
    - [Git для инженеров-тестировщиков](#git-для-инженеров-тестировщиков)
  - [Первичная настройка Git](#первичная-настройка-git)
    - [Особенности настройки в Windows](#особенности-настройки-в-windows)
  ...
  - [Работа с удаленными репозиториями](#работа-с-удаленными-репозиториями)
    - [Сохранение логина и пароля](#сохранение-логина-и-пароля)
  ...
  - [Немного о ветках](#немного-о-ветках)
    - [Слияние веток](#слияние-веток)
      - [Конфликт слияния (Merge conflict)](#конфликт-слияния-merge-conflict)
  - [Теги](#теги)
  - [Разные проблемы](#разные-проблемы)
    - [Не работает .gitignore](#не-работает-gitignore)
    - [В разных терминалах разный `status`](#в-разных-терминалах-разный-status)
...
![Git commands](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/git.jpg?raw=true)
...
![Git](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/git.png?raw=true)
...
![Git stage](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/git_stage.png?raw=true)
...
![modifying files](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/modifing_files.png?raw=true)
...
`git revert` — это команда Git, которая создаёт новый коммит, отменяющий изменения, внесённые предыдущим коммитом. Это безопасный способ откатить изменения, потому что история коммитов не переписывается — просто сверху добавляется новый «отменяющий» коммит.

`git revert <хэш-коммита>` - создаёт новый коммит, который отменяет изменения, внесённые указанным коммитом.

`git revert HEAD` - создаётся новый коммит, который отменяет изменения из последнего коммита.

Отменить несколько коммитов и зафиксировать одним коммитом:

```sh
git revert -n ABC123..HEAD   # отменить всё, что идёт после ABC123 (без фиксации)
# теперь рабочая копия и индекс как на момент ABC123
# можно сделать коммит:
git commit -m "Состояние на ABC123"
```
...
`git config --global credential.helper store` - включает хранение логина и пароля в файле.  
ВНИМАНИЕ!!! пароль хранится в открытом виде.

`git config --get credential.helper` - посмотреть, какой helper используется
...
Если в репозитории уже были файлы возможно придется вначале выполнить команду:  
`git pull origin main --allow-unrelated-histories` (при этом желательно, чтобы в удаленном репозитории не было файлов)
...
![branch](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/branch.png?raw=true)
...
Если не работает (например, в отслеживаемых файлах уже были изменения до добавления их в .gitignore):  
`git reset HEAD --` - сбрасываем изменения
