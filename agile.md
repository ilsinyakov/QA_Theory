...
* [Виды гибкой разработки](#виды-гибкой-разработки)
  ...
  * [XP (Extreme Programming)](#xp-extreme-programming)
    * [Ключевые ценности](#ключевые-ценности)
    * [Основные практики](#основные-практики)
    * [Когда XP подходит и ограничения](#когда-xp-подходит-и-ограничения)
...
![Каскадная модель](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/cascade_model.png?raw=true)
...
* В рамках методологии требуется подробная документация уже на начальном этапе проекта.   Много времени посвящается тому, чтобы определить, что В ТОЧНОСТИ хочется получить, и как это будет реализовано.
...
![Agile](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/agile.png?raw=true)
...
Этот метод подходит в ситуациях, когда невозможно спрогнозировать, что именно нужно будет сделать, и когда требуется быстрое внедрение без необходимости заботиться о других аспектах работы. Метод используется для построения рабочего процесса, управления им и оптимизации. Канбан позволяет визуализировать ход работ и снизить количество незавершённых задач. В методе Канбан спринты не используются.
...
![Scrum](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/scrum.png?raw=true)
...
**XP — Extreme Programming** (буквально — «экстремальное программирование») — одна из реализаций Agile-подхода к разработке ПО. Основная идея — максимально быстрый и качественный выпуск рабочих инкрементов за счёт набора практик, тесной коммуникации и непрерывного улучшения.

XP фокусируется на коротких итерациях с частыми релизами, тесном общении с заказчиком и на инженерных практиках, которые минимизируют баги и технический долг (TDD, рефакторинг, CI). Команда работает в паре, разделяет ответственность за код и регулярно демонстрирует рабочий продукт. Решения принимаются итеративно: меняется то, что приносит ценность прямо сейчас. XP призвана делать разработку предсказуемой, гибкой и устойчивой в условиях меняющихся требований.

#### Ключевые ценности

* **Коммуникация** — команда общается часто и прямо.
* **Простота** — делать самое простое решение, которое работает.
* **Обратная связь** — от тестов, заказчика, ретроспектив.
* **Мужество (Courage)** — быстро исправлять ошибки и менять код.
* **Уважение** — к коллегам и к качеству.

#### Основные практики

* **Парное программирование (pair programming)**
* **TDD** — разработка через тестирование (unit tests → код → рефакторинг)
* **Непрерывная интеграция (CI)** — частые коммиты и сборки.
* **Рефакторинг** — постоянное улучшение дизайна кода.
* **Малые релизы / короткие итерации** (обычно 1–2 недели)
* **Клиент на месте (on-site customer)** — быстрые ответы на вопросы по приоритетам/приёмке.
* **Коллективная собственность кода** — любой может править любой модуль.
* **Планирование через истории (user stories).

#### Когда XP подходит и ограничения

Подходит, если требования часто меняются, команда небольшая и коммуникация живая, важна скорость поставки и качество кода.

Может не подойти в очень больших распределённых командах без хорошей дисциплины, или там, где невозможно иметь постоянного представителя заказчика.
