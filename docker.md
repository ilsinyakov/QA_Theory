...
* [О контейнеризации](#о-контейнеризации)
...
`sudo docker start testuser-3` - повторный запуск ранее запущенного контейнера
...
![Docker Layer](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/docker_layer.jpg?raw=true)
...
`docker system df` - проверить, сколько места на диске занимают образы, контейнеры, тома и кэш.

`sudo docker rmi [<image id> <image id> ...]` - удалить образы из системы  
`-f` - принудительно  
