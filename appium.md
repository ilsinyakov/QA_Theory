...
![Espresso](https://github.com/ilsinyakov/QA_Theory/blob/main/Pictures/Espresso.png?raw=true)
...
public void espressoTest() {
    onView(allOf(allOf(withId(R.id.label_bf_hotel_name), 
...
Для установки требуется установленный Node.js. Если он установлен достаточно набрать:

`npm install -g appium`

Можно сразу установить appium-doctor - инструмент, который показывает, что в системе настроено не так для работы с Appium:

`npm install @appium/doctor --location=global`

### Установка драйвера

Для работы Appium нужен соответствующий драйвер. Например, для Android:

`appium driver install uiautomator2`

### Android Studio

Для создания виртуальных устройств, для работы с ADB ставим Android Studio

`id` подключенного устройства можно взять из вывода команды:

`adb devices`

### Настройка окружения

В переменные окружения должна быть прописана переменна `JAVA_HOME`.  
Для Windows следующая команда в ps от админа:

`setx JAVA_HOME "C:\Program Files\Java\jdk-24" /M`

Также нужно прописать `ANDROID_HOME`:

`setx ANDROID_HOME "c:\Users\ilsin\AppData\Local\Android\Sdk\" /M`

Для проверки правильности настройки окружения и софта можно использовать `appium-doctor`. Например, для Android:

`appium-doctor --android`

В переменную PATH нужно прописать путь к тулзам:

`setx PATH "%PATH%;%ANDROID_HOME%\platform-tools" /M`

Посмотреть переменные окружения в ps можно:

`Get-ChildItem Env:`

### Appium Inspector

Для поиска локаторов на подключенном устройстве нужно установить Appium Inspector.
